...
template <typename T>
void
Cdynamicarray<T>::remove(unsigned int index)
{
  if (!Cdynamicarray::n)
    return;
...
template <typename T>
void
Cdynamicarray<T>::insert(const T& t, unsigned int index)
{
...
template <typename T>
void
Cdynamicarray<T>::resize(unsigned int length)
{
  Carray<T> tmp(length);
...
template <typename T>
T*
Cdynamicarray<T>::split(unsigned int index)
{
  T* t;
